...
class StaffPerformanceShowTest extends TestCase
{
    ...
    public function test_target_metrics_are_isolated_to_the_targets_own_restaurant_even_when_owner_spans_many(): void
    {
        ...
        $this->actingAs($owner, 'web')
            ->getJson("/api/v1/staff/{$waiterA->id}/performance")
            ->assertOk()
        ...
        $this->actingAs($owner, 'web')
            ->getJson("/api/v1/staff/{$waiterB->id}/performance")
            ->assertOk()
    ...
    public function test_staff_of_another_organization_is_not_found(): void
    {
        ...
        $this->actingAs($owner, 'web')
            ->getJson("/api/v1/staff/{$otherStaff->id}/performance")
            ->assertNotFound();
    }
    ...
    public function test_invalid_performance_period_is_rejected(): void
    {
        ...
        $this->actingAs($owner, 'web')
            ->getJson("/api/v1/staff/{$waiter->id}/performance?from=not-a-date&to=2026-01-01")
            ->assertStatus(422)
